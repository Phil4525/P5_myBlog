            ...
            <nav aria-label="Pagination">
                ...
                <ul class="pagination justify-content-center my-4">
                    <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
                        <a class="page-link" href="index.php?action=adminSearch&keyword=<?= urlencode($keyword) ?>&page=<?= urlencode($currentPage) - 1 ?>#results" aria-label="Previous">
                            <span class="fa-solid fa-arrow-left" aria-hidden="true"></span>
                        </a>
                    </li>
                    <?php for ($page = 1; $page <= $pages; $page++) : ?>
                        <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>" aria-current="page">
                            <a class="page-link" href="index.php?action=adminSearch&keyword=<?= urlencode($keyword) ?>&page=<?= urlencode($page) ?>#results"><?= $page ?></a>
                        </li>
                    <?php endfor ?>
                    <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
                        <a class="page-link" href="index.php?action=adminSearch&keyword=<?= urlencode($keyword) ?>&page=<?= urlencode($currentPage) + 1 ?>#results" aria-label="Next">
